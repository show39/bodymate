class UsersController < ApplicationController
  def show
    ...
    @entry_tickets = Booking.where('user_id = ?', @user.id).includes(ticket: [:event]).order('created_at DESC').limit(10)
  end
  ...
  def ticket
    @user = User.find(params[:id])
    @tickets = Booking.where('user_id = ?', @user.id).includes(ticket: [:event]).order("created_at desc").page(params[:page]).per(24)
  end
end
